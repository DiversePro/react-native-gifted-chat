...
export default class MessageContainer extends React.PureComponent {
    constructor() {
        ...
        this.handleOnScroll = (event) => {
            ...
            this.props.setScrollPosition && this.props.setScrollPosition(contentOffsetY);
        ...
        this.renderRow = ({ item, index }) => {
            ...
            if (!item.messageId && item.messageId !== 0) {
                warning('GiftedChat: `messageId` is missing for message', JSON.stringify(item));
            }
            if (!item?.createdBy) {
                if (!item.system) {
            ...
            if (messages && user) {
                ...
                const messageProps = {
                    ...
                    inverted,
                    nextMessage,
                    position: item?.createdBy?.attorneyId === user.id ? 'right' : 'left',
                };
    ...
    render() {
        ...
        <FlatList  ref={this.props.forwardRef} extraData={[this.props.extraData, this.props.isTyping]} keyExtractor={this.keyExtractor} enableEmptySections automaticallyAdjustContentInsets={false} inverted={inverted} data={this.props.messages} style={[styles.listStyle,{marginBottom:this.props.position=="absolute" ? !this.props.isReplyActive ? RFValue(14) + this.props?.extraMargin: 0 :!this.props.isReplyActive ? this.props?.extraMargin:0}]} contentContainerStyle={styles.contentContainerStyle} renderItem={this.renderRow} {...this.props.invertibleScrollViewProps} ListEmptyComponent={this.renderChatEmpty} ListFooterComponent={inverted ? this.renderHeaderWrapper : this.renderFooter} ListHeaderComponent={inverted ? this.renderFooter : this.renderHeaderWrapper} onScroll={this.handleOnScroll} scrollEventThrottle={100} onLayout={this.onLayoutList} onEndReached={this.onEndReached} onEndReachedThreshold={0.1} {...this.props.listViewProps} windowSize={5} removeClippedSubviews={true} maxToRenderPerBatch={5} updateCellsBatchingPeriod={200} initialNumToRender={5}/>
        {this.props.scrolling && <View
          style={{
            position: 'absolute',
            top: 0,
            bottom: 0,
