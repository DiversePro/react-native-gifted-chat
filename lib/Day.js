...
const covertTimeToUTC = (time)=>{
  let newTime  = ""
  if(time?.length){
      let lastValue = time[time.length - 1];
      if(lastValue == "z" || lastValue == "Z"){
          newTime = time;
      } else {
          newTime = `${time}Z`;
      }
...
const dateHandler=( msgdatepassed)=>{

let nowDate = new Date();
let msgdate = new Date(covertTimeToUTC(msgdatepassed));
